...
def parse_and_save(url):
    ...
    with Session(engine) as session:
        ...
        session.add(TaskCategoryLink(task_id=task.id, category_id=session.exec(select(Category).where(Category.name == 'web parsing')).first().id))
        session.commit()
...
def main():
    threads = []
    for chunk in [URLS[i:i + 2] for i in range(0, 8, 2)]:
        t = Thread(target=lambda: [parse_and_save(url) for url in chunk])
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
